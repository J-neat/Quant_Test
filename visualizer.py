...
def plot_stock_chart(df, ticker, score, reasons, fundamentals=None):
    ...
    print(f"  📊 [{ticker}] 차트 및 재물대 정보 시각화 중...")
    ...
    # 💡 [핵심 시각화] 차트 배경에 매물대(Volume Profile) 가로로 그리기
    try:
        ax_vp = ax1.twiny() # x축을 공유하는 보조 축 생성
        ...
        ax_vp.set_xlim(0, hist.max() * 4) # 막대가 차트의 1/4 정도만 차지하도록 제한
        ax_vp.axis('off') # 보조 축의 눈금은 숨김
